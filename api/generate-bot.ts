import { fillBotTemplate, BotParams } from './_lib/bot-template';
...
// Plain Vercel serverless function handler (this project is a static
// Rsbuild/React Router SPA, not Next.js — so this uses the framework-agnostic
// Vercel Node function signature, not NextRequest/NextResponse).
export default async function handler(req: any, res: any) {
    ...
    try {
        ...
        res.status(500).json({ error: err?.message || 'Failed to generate bot' });
    }
}
